...
const PropertyCard: React.FC<PropertyCardProps> = ({ property }) => {
  ...
  const InfoChip: React.FC<{ icon: React.ReactNode; text: string | number }> = ({ icon, text }) => (
    <div className="flex items-center space-x-2 space-x-reverse bg-gray-700 rounded-full px-3 py-1 text-sm text-gray-300">
      {icon}
      <span>{text}</span>
    </div>
  );
  ...
  return (
    <div className="bg-gray-900 border border-gray-700 rounded-xl overflow-hidden shadow-lg hover:shadow-cyan-500/20 hover:border-cyan-500/50 transition-all duration-300 transform hover:-translate-y-1 h-full flex flex-col">
      <div className="relative">
        ...
        <div className="absolute top-2 left-2 bg-black/60 text-white text-xs font-bold px-3 py-1 rounded-full">
          {type === 'sale' ? 'فروش' : 'اجاره'}
        </div>
        <button className="absolute top-2 right-2 p-2 bg-gray-900/80 rounded-full hover:bg-gray-900 transition-colors">
      ...
      <div className="p-4 flex flex-col flex-grow">
        ...
        <p className="text-gray-400 text-sm mb-4 flex items-center">
          <MdLocationOn size={16} className="ml-1" />
          {location}
        </p>
